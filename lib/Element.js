...
Element.prototype._toReact = function (index, preprocessor) {
  index = index || 0
  ...
  const pair = preprocessor.call(this, this.nodeName, props)

  return React.createElement(pair.component, pair.props, this.text || this.children.map(function (el, i) {
    if (el instanceof Element) {
      return el._toReact(i, preprocessor)
    } else {
      return el
    }
  }))
...
Element.prototype.toReact = function (index, createFunction) {
  return this._toReact(index, function (nodeName, props) {
    return {
      component: nodeName,
      props: props
    }
...
Element.prototype.toReactNative = function (index, componentMapping) {
  return this._toReact(index, function (nodeName, props) {
    var component = componentMapping[nodeName]

    if ('transform' in props) {
      var value = props.transform
      if (typeof value === 'string') {
        var m = value.match(/translate\((.+?)\)/)
        if (m) {
          var p = m[1].split(',')
          props.transform = {
            translateX: p[0],
